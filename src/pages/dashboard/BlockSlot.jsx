...
const BlockSlot = ({ user }) => {
  ...
  // Helper function to generate 30-minute slots from time ranges
      ...
      while (currentHour < endHour || (currentHour === endHour && currentMin < endMin)) {
        ...
        if (nextMin >= 60) {
          ...
          const slotStart = `${currentHour.toString().padStart(2, "0")}:${currentMin
            .toString()
            .padStart(2, "0")}`;
          const slotEnd = `${nextHour.toString().padStart(2, "0")}:${adjustedNextMin
            .toString()
            .padStart(2, "0")}`;
  ...
  const handleDayChange = (dayValue) => {
    ...
    // Use regex to extract all time strings like "2:00 PM" or "3:00 AM"
    ...
    // Block slots that match any of these extracted start times
    const blocked = allTimeSlots.filter((slot) => {
      const slotStart = slot.split(" - ")[0].replace(/\s+/g, " ").toUpperCase().trim();
      return matches.includes(slotStart);
    });
  ...
  const handleSlotToggle = (slot) => {
    if (selectedSlots.includes(slot)) {
      ...
      if (selectedSlots.length >= 4) {
        toast.warning("You can block a maximum of 4 slots.");
        return;
      }
  ...
  const renderTimeOption = (label, timeData, value, id) => {
    ...
    // Format the display text to show time ranges properly
  ...
  try {
    const blockedSlotsString = selectedSlots.join(" - "); // or use format your backend expects
    ...
    const response = await fetch("http://localhost:5000/api/dashboard/updateBlockSlots", {
      ...
      body: JSON.stringify({
        ...
        day: dayMap[selectedDay],       // e.g., "mon", "tue"
          ...
          <div className="border border-gray-300 rounded p-4 bg-gray-50 min-h-[200px] flex flex-col">
            ...
            <div className="mt-auto pt-3 border-t border-gray-200 flex justify-end">
              <button
                ...
                className="bg-blue-600 text-white px-5 py-2 rounded hover:bg-blue-700 transition-colors text-sm flex items-center space-x-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
              >
                <Save className="w-4 h-4" />
                <span>Save Blocked Slots</span>
              </button>
            </div>
          </div>
        ) : (
